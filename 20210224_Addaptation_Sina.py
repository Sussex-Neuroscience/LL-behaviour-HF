...
win2 = visual.Window(fullscr=True, monitor=mon, screen = 2 , waitBlanking=False, useFBO = True)   #waitBlanking=False if you want 120Hz
grating2 = visual.GratingStim(win2, units='deg', mask='circle', sf = info['SF'], tex='sin', pos = info['Position'])    # maskParams = {'sd':3} for mask 'gauss' and maskParams = {'fringeWidth':0.2} for 'raisedCos'
grating2.phase = 0.5
warper2 = Warper(win2, warp='spherical', warpfile="", warpGridsize  = 128, eyepoint = [0.5,0.5], flipHorizontal = False, flipVertical = False)
...
for thisTrial in range(info['trialN']):
    ...
    for frameN in range(info['StimLength']):
        ...
        gratingran.setPhase(info['TF']/info['RefreshRate'], '+')           # To get the TF in Hz, divide TF by the refresh rate       # set like this for TF fction of speed:  grating.setPhase(mouse_dX/100,'+')
